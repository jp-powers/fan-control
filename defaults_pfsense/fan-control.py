...
## ipmi-fan-control.py
##
## Purpose: Set fan speeds of IPMI controlled fans based on CPU & HDD temperature(s)
##
# import needed components
import time # used for sleep at end
...
### define log basics
# logging is fairly limited so probably leave level= alone?
...
def logger_start(): # show when logging of fan-control sessions starts
    ...
    logging.info("||                         ||")
    logging.info("|| assuming direct control ||")
    logging.info("||                         ||")
    logging.info("=============================")
...
def get_hdd_temp(disk_list): # this feels like a silly way to do it but it works for me?
...
def get_cpu_zone_speed(temp,cpu_fan_curve): # based on the fan curve set above, decide what the appropriate fan power level (fan speed) should be, return it as an integer.
    i = 0 # create an iterator
    power = 100 # power defaults to 100% in event of any math being super broken from the fan curve
...
def get_hdd_zone_speed(temps,max_temp,speed_addition,hdd_fan_curve):# based on the fan curve set above, decide what the appropriate fan power level (fan speed) should be, return it as an integer.
    i = 0 # create an iterator
    power = 100 # power defaults to 100% in event of any math being super broken from the fan curve
    ...
    if temps[1] >= max_temp: # if current max temp is greater than config's max temp bump the returned power by our max addition
        hdd_panic = int(power) + speed_addition
        return hdd_panic
